...
function normaliseLensKey(value = '') {
  return normaliseText(value).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
}
...
async function buildLiveDraft(input = {}, config = getConfig()) {
  ...
  try {
    ...
    const lensKey = normaliseLensKey(parsed?.scenarioLens?.key || fallbackLensKey) || fallbackLensKey;
    if (fallbackLensKey !== 'general-enterprise-risk' && lensKey !== fallbackLensKey) {
      throw new Error(`Live draft lens drifted from ${fallbackLensKey} to ${lensKey || 'unknown'}.`);
    }
